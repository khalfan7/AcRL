...
_M2S = {12: 'Winter', 1: 'Winter', 2: 'Winter', 3: 'Spring', 4: 'Spring', 5: 'Spring', 6: 'Summer', 7: 'Summer', 8: 'Summer', 9: 'Fall', 10: 'Fall', 11: 'Fall'}
_SEASON_ORDER = ['Winter', 'Spring', 'Summer', 'Fall']

def _fmt(s: pd.Series) -> str:
    return f'{s.mean():.3f} +/- {s.std():.3f}'

def run_generalization_test(algo_cls, algo_name: str, *, n_episodes: int=100, env_factory=None, city_name: str='Albany'):
    ...
    if not model_path.exists():
        raise FileNotFoundError(f'No model in {LOG_DIR}. Run train_{algo_name.lower()}.py first.')
    ...
    for ep in range(n_episodes):
        ...
        energy = float(np.sum(np.abs(pwr_arr)) * dt_ctrl / 3600000.0)
        tot_r = float(rew_arr.sum())
        season = _M2S[ep_month]
        peak_w = float(np.max(np.abs(pwr_arr)))
        slew = float(np.mean(np.abs(np.diff(act_arr))))
        records.append(dict(episode=ep, season=season, month=ep_month, doy=ep_doy, mae=mae, rmse=rmse, violations_pct=viol05, within_1c_pct=100.0 - viol10, energy_kwh=energy, reward=tot_r, mean_slew=slew, peak_power_w=peak_w))
        traces.append(dict(time=t_arr.copy(), indoor=in_arr.copy(), outdoor=out_arr.copy(), power=pwr_arr.copy(), action=act_arr.copy(), ghi=ghi_arr.copy(), wind=wnd_arr.copy(), month=ep_month, doy=ep_doy, season=season))
        if (ep + 1) % 25 == 0 or ep == 0:
            print(f'  Ep {ep + 1:3d}/{n_episodes}  {season:6s}  MAE {mae:.3f}  Viol {viol05:5.1f}%  Energy {energy:.2f} kWh')
    env.close()
    ...
    print(f'  {'-' * 66}')
    print(f'  {'Season':<8s}  {'n':>3s}  {'MAE(C)':>14s}  {'RMSE(C)':>14s}  {'Viol(%)':>14s}  {'Energy(kWh)':>14s}')
    for s in _SEASON_ORDER:
        sub = df[df['season'] == s]
        if sub.empty:
            continue
        print(f'  {s:<8s}  {len(sub):>3d}  {_fmt(sub['mae']):>14s}  {_fmt(sub['rmse']):>14s}  {_fmt(sub['violations_pct']):>14s}  {_fmt(sub['energy_kwh']):>14s}')
    print(f'  {'-' * 66}\n')
    trace_arrays = {'time': np.array([t['time'] for t in traces]), 'indoor': np.array([t['indoor'] for t in traces]), 'outdoor': np.array([t['outdoor'] for t in traces]), 'power': np.array([t['power'] for t in traces]), 'action': np.array([t['action'] for t in traces]), 'ghi': np.array([t['ghi'] for t in traces]), 'wind': np.array([t['wind'] for t in traces]), 'month': np.array([t['month'] for t in traces]), 'doy': np.array([t['doy'] for t in traces])}
...
def run_train_vs_test(algo_cls, algo_name: str, *, n_episodes: int=100):
    ...
    print('=' * 70)
    df_train = run_generalization_test(algo_cls, algo_name, n_episodes=n_episodes, env_factory=make_train_env, city_name='Syracuse (train)')
    print('\n' + '=' * 70)
    print(f'  {algo_name}  --  Test city (Albany)')
    print('=' * 70)
    df_test = run_generalization_test(algo_cls, algo_name, n_episodes=n_episodes, env_factory=make_test_env, city_name='Albany (test)')
    metrics = [('mae', 'MAE (°C)'), ('rmse', 'RMSE (°C)'), ('violations_pct', 'Comfort Violations (%)'), ('energy_kwh', 'Energy (kWh/day)')]
    print(f'\n{'=' * 70}')
