...
ALGO_LABELS = {'PPO_nl': 'PPO', 'A2C_nl': 'A2C', 'SAC_nl': 'SAC', 'TD3_nl': 'TD3'}
ALGO_COLORS = {'PPO_nl': '#1f77b4', 'A2C_nl': '#ff7f0e', 'SAC_nl': '#2ca02c', 'TD3_nl': '#d62728'}
ALGO_STYLES = {'PPO_nl': '-', 'A2C_nl': '--', 'SAC_nl': '-.', 'TD3_nl': ':'}
SEASONS = ['Winter', 'Spring', 'Summer', 'Fall']
MAIN_METRICS = ['mae', 'comfort_in_band_pct', 'energy_kwh']
MAIN_LABELS = {'mae': 'MAE (°C)', 'comfort_in_band_pct': 'Time within ±0.5°C (%)', 'energy_kwh': 'Energy (kWh/day)'}
_HIGHER_IS_WORSE = {'mae': True, 'comfort_in_band_pct': False, 'energy_kwh': True}
FMT = {'mae': '{:.2f}', 'comfort_in_band_pct': '{:.0f}', 'energy_kwh': '{:.1f}'}
RANGES = {'mae': (0.0, 1.5), 'comfort_in_band_pct': (70.0, 100.0), 'energy_kwh': (0.0, 60.0)}
...
def _load_csvs() -> pd.DataFrame:
    frames = []
    for algo in ALGOS:
        for city_tag, city_label in [('albany (test)', 'Albany'), ('syracuse (train)', 'Syracuse')]:
...
def _load_convergence() -> dict:
    ...
    for algo in ALGOS:
        ...
        if not path.exists():
            continue
        npz = np.load(path)
        data[algo] = {'timesteps': npz['timesteps'], 'results': npz['results'], 'mean': npz['results'].mean(axis=1)}
    return data
...
def _fig_albany_summary(df: pd.DataFrame) -> None:
    ...
    for ai, algo in enumerate(avail):
        for si, s in enumerate(SEASONS):
            vals = albany[(albany['algo'] == algo) & (albany['season'] == s)][col].values
            if len(vals) == 0:
                continue
    ...
    mn, mx = RANGES[col]
    normed = np.where(np.isnan(mat), 0.3, np.clip((mat - mn) / max(mx - mn, 1e-09), 0.0, 1.0))
    cmap = plt.get_cmap(cmap_name)
    for ai in range(n_algos):
        for si in range(n_seasons):
            rgba = cmap(0.15 + 0.7 * normed[ai, si])
            ax.add_patch(plt.Rectangle((si, ai), 1, 1, facecolor=rgba, edgecolor='white', linewidth=2))
            lum = 0.299 * rgba[0] + 0.587 * rgba[1] + 0.114 * rgba[2]
            tc = 'white' if lum < 0.55 else 'black'
            ax.text(si + 0.5, ai + 0.5, text[ai][si], ha='center', va='center', fontsize=12, fontweight='bold', color=tc)
    ...
    ax.set_title('Albany — MAE by Season (°C)\n(median over 25 episodes per season)', fontsize=12, fontweight='bold', pad=14)
    fig.tight_layout()
...
def _fig_generalization_gap(df: pd.DataFrame) -> None:
    ...
    for algo in ALGOS:
        ...
        for m in MAIN_METRICS:
            s_ref = syr[m].median()
            a_ref = alb[m].median()
            raw_pct = 0.0 if abs(s_ref) < 1e-09 else (a_ref - s_ref) / abs(s_ref) * 100.0
    ...
    im = ax.imshow(mat, cmap='RdYlGn_r', aspect='auto', vmin=-max_abs, vmax=max_abs)
    ...
    ax.set_title('Generalization Gap (Syracuse → Albany)', fontsize=12, fontweight='bold', pad=14)
    fig.tight_layout()
...
def _fig_best_traces(df: pd.DataFrame, best_algo: str) -> None:
    ...
    fig, (ax_t, ax_p) = plt.subplots(1, 2, figsize=(14, 4.5), gridspec_kw={'wspace': 0.28})
    fig.suptitle(f'Winter vs Summer on Albany ({label})', fontsize=13, fontweight='bold')
    months = traces['month']
    for season in show_seasons:
        ...
        ax_t.plot(time_arr, in_arr, color=clr, lw=2.2, label=f'{season} indoor (MAE {rec['mae']:.2f}°C, {comfort_pct:.0f}% comfort)')
        ax_t.plot(time_arr, out_arr, color=clr, lw=1.2, ls='--', alpha=0.5, label=f'{season} outdoor')
        ax_p.plot(time_arr, pwr_arr, color=clr, lw=1.8, label=f'{season} ({rec['energy_kwh']:.1f} kWh/day)')
    ax_t.axhline(setpoint, color='green', lw=1.5, ls=':', label='Setpoint')
    ax_t.fill_between([0, 24], setpoint - 0.5, setpoint + 0.5, color='green', alpha=0.1, label='±0.5°C band')
...
def _fig_convergence(conv: dict) -> None:
    ...
    for algo in ALGOS:
        ...
        for j in range(raw.shape[1]):
            ax.scatter(ts, raw[:, j], color=ALGO_COLORS[algo], alpha=0.12, s=8, zorder=1, linewidths=0)
        ax.plot(ts, mean, color=ALGO_COLORS[algo], linestyle=ALGO_STYLES[algo], linewidth=2.2, label=ALGO_LABELS[algo], zorder=3)
...
def generate_report(best_algo: str='TD3_nl'):
    ...
    if df.empty:
        ...
        return
    print(f'  {len(df)} rows — {df['algo'].nunique()} algos × {df['city'].nunique()} cities\n')
...
if __name__ == '__main__':
    ...
    p.add_argument('--best', type=str, default='TD3_nl', help='Algorithm for the behaviour trace figure (default: TD3_nl)')
    args = p.parse_args()
