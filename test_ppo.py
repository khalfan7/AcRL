"""
Daily Episode Profile — PPO Agent (Testing Phase)
Runs the trained PPO agent on a single 24-hour day and plots:
...
LOG_DIR = os.path.join(_HERE, "results", "PPO")

model_path = os.path.join(LOG_DIR, "best_model.zip")
if not os.path.exists(model_path):
    model_path = os.path.join(LOG_DIR, "final_model.zip")
if not os.path.exists(model_path):
    raise FileNotFoundError(f"Model not found in {LOG_DIR}. Run train_ppo.py first.")
...
dt_ctrl   = sim.control_timestep          # seconds per control step
n_steps   = sim.max_steps

# Pre-compute the outdoor-temp profile aligned to the physics step.
# get_state() reads outdoor_temp_array[current_step] AFTER incrementing,
# so it is shifted by one index.  Reading the profile directly avoids that.
...
# Comfort-band violations (±0.5 °C)
outside_band = np.abs(indoor_arr - setpoint) > 0.5
comfort_viol = outside_band.sum() / step_count * 100

# Cooling  → positive power   (heat removed from zone)
# Heating  → negative power   (heat added to zone)
...
# ── Bottom: HVAC power ─────────────────────────────────────────────────────
#   +ve power → cooling  (heat removed from zone)
#   −ve power → heating  (heat added to zone)
...
out_path = os.path.join(LOG_DIR, "daily_episode_profile_ppo.png")
