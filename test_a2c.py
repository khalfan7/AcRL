"""
Daily Episode Profile — A2C Agent (Testing Phase)
Runs the trained A2C agent on a single 24-hour day and plots:
...
LOG_DIR = os.path.join(_HERE, "results", "A2C")

model_path = os.path.join(LOG_DIR, "best_model.zip")
if not os.path.exists(model_path):
    model_path = os.path.join(LOG_DIR, "final_model.zip")
if not os.path.exists(model_path):
    raise FileNotFoundError(f"Model not found in {LOG_DIR}. Run train_a2c.py first.")
...
out_path = os.path.join(LOG_DIR, "daily_episode_profile_a2c.png")
